function [] = tdoa_position
% modified to simulate tdoa position of target emitter used for spectrum monitoring
% original is PRSPositioningExample.m in LTE System Toolbox example

%%
% ######################## begin of my code #############

rf_sensor_length = 4;   % number of rf sensor
...
hPositioningPlotPositions(rf_sensor);
...
for i = 1:rf_sensor_length
    ...
    rx{i} = [zeros(sampleDelay(i), 1); tx{1}; ... 
...
%     % copy from previous statements to compare above statements
%     grid = lteDLResourceGrid(enb{i});             % Empty resource grid
%     grid(ltePRSIndices(enb{i})) = ltePRS(enb{i}); % Map PRS REs
%     tx{i} = lteOFDMModulate(enb{i}, grid);        % OFDM modulate
